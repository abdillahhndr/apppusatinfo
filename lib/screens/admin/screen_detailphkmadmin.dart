...
class DetailPhkmAdmin extends StatefulWidget {
  final Datum? data;

  const DetailPhkmAdmin(this.data, {Key? key}) : super(key: key);

  @override
  State<DetailPhkmAdmin> createState() => _DetailPhkmAdminState();
}

class _DetailPhkmAdminState extends State<DetailPhkmAdmin> {
      ...
      if (data.isSuccess == true) {
        setState(() {
          ...
          Navigator.pushAndRemoveUntil(
              context,
              MaterialPageRoute(builder: (context) => const ListPhkmAdmin()),
              (route) => false);
        });
                      ...
                      child: Column(
                        ...
                        children: <Widget>[
                            ...
                            builder: (context, snapshot) {
                              if (snapshot.connectionState ==
                                ...
                                if (snapshot.hasData) {
                                  ...
                                  return Text('Gagal memuat gambar Laporan');
                                }
                              } else {
                                return Center(
                                    child: CircularProgressIndicator());
                          ...
                          ElevatedButton(
                            onPressed: () {
                              openPDF(context,
                                  '$url/uploads/ktp/${widget.data!.fotoLaporan}');
                            },
                            style: ElevatedButton.styleFrom(
                              padding: EdgeInsets.symmetric(
                                  horizontal: 16, vertical: 8),
