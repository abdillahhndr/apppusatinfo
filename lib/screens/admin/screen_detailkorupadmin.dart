...
class DetailKorupAdmin extends StatefulWidget {
  final Datum? data;

  const DetailKorupAdmin(this.data, {Key? key}) : super(key: key);

  @override
  State<DetailKorupAdmin> createState() => _DetailKorupAdminState();
}

class _DetailKorupAdminState extends State<DetailKorupAdmin> {
  ...
  Future<ModelAddjms?> approve(String idj) async {
    try {
      ...
      http.Response res =
          await http.post(Uri.parse('$url/korupapprovereject.php'), body: {
        "id": idj,
        "status": 'approve',
      });
      ...
      if (data.isSuccess == true) {
        setState(() {
          ...
          Navigator.pushAndRemoveUntil(
              context,
              MaterialPageRoute(builder: (context) => const ListKorupAdmin()),
              (route) => false);
        });
